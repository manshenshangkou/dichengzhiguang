...
public class SwordGirlScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!mStateInfo.IsName(IdleState) && mStateInfo.normalizedTime > 1.0F/*动画播放的程度 0-1*/)
        {
    ...
    void Attack()
    {
        ...
        else if (mStateInfo.IsName(Attack3State) && mHitCount == 3 && mStateInfo.normalizedTime > 0.80F)
        {
